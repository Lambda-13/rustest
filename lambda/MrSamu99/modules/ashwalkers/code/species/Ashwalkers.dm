...
/datum/component/ash_age/proc/check_evolution()
	...
	switch(current_stage)
		...
		if(3)
			/*var/obj/item/bodypart/l_arm/left_arm = human_target.get_bodypart(BODY_ZONE_L_ARM)
			if(left_arm)
				left_arm.unarmed_damage_low += 5
				left_arm.unarmed_damage_high += 5

			var/obj/item/bodypart/r_arm/right_arm = human_target.get_bodypart(BODY_ZONE_R_ARM)
			if(right_arm)

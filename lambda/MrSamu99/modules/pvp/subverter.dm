...
/obj/machinery/subverter/proc/can_subvert(datum/overmap/ship/controlled/target_ship, antag = FALSE)
	...
	if (target_ship.docked_to != null)
		return SUB_TARGET_DOCKED
	if (target_ship.docking == TRUE)
...
/obj/machinery/subverter/proc/attempt_to_subvert(datum/overmap/ship/controlled/target_ship)
	...
	if (!target_ship.run_antivirus())
		...
		var/list/O = ship.get_nearby_overmap_objects()
		if (target_ship.x == ship.x && target_ship.y == ship.y)
			for (var/datum/overmap/add in get_turf(ship))
				if (add == ship)
					continue
				LAZYOR(O, add)
			LAZYREMOVE(O, target_ship) //prevent confusion, since you wanna dock in the empty space
...
/obj/machinery/subverter/proc/adjust_hacked(state)
	...
	var/ship_min_dist
	for (var/datum/overmap/ship/controlled/ship_in_view in view(world.view, get_turf(ship)))

...
/obj/machinery/computer/autopilot/ui_interact(mob/user, datum/tgui/ui)
	//if(isliving(user) || !isliving(user) || isAdminGhostAI(user))
	if(!ship && !reload_ship())
		return
...
/obj/machinery/computer/autopilot/ui_data(mob/user)
	var/list/data = list()
	var/obj/docking_port/mobile/mobile = ship.shuttle_port
	...
	for(var/datum/overmap/object in view(world.view, get_turf(ship)))
		if((object.x == ship.x && object.y == ship.y) || istype(object, /datum/overmap/event) || object == ship)
			continue
	...
	*/
	...
	for(var/datum/overmap/ship/controlled/flying_ship in view(world.view, get_turf(ship)))
		if(flying_ship == ship)
			continue
		var/list/location_data = list(
			id = REF(flying_ship),

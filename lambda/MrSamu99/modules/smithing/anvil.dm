...
/obj/structure/anvil
	...
	icon = 'lambda/MrSamu99/icons/smithing/smith.dmi'
	icon_state = "anvil"
	density = TRUE
	anchored = TRUE
...
/obj/structure/anvil/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/ingot))
		...
		if(notsword.workability == "shapeable")
			...
			currentquality = anvilquality
			var/skillmod = user.mind.get_skill_level(/datum/skill/blacksmithing) * 1.5
...
/obj/structure/anvil/proc/do_shaping(mob/user, var/qualitychange)
	...
	var/steptime = 50
	var/skillmod = user.mind.get_skill_level(/datum/skill/blacksmithing) * 3 / 7 
	if(skillmod != 0)
		steptime = 50 / skillmod
	playsound(src, 'lambda/MrSamu99/sound/smithing/clang2.ogg',40, 2)
	...
	playsound(src, 'lambda/MrSamu99/sound/smithing/clang2.ogg',40, 2)
	addtimer(CALLBACK(GLOBAL_PROC, .proc/playsound, src, 'lambda/MrSamu99/sound/smithing/clang2.ogg', 40, 2), 15)
...
/obj/structure/anvil/proc/tryfinish(mob/user)
	...
	var/combinedqualitymax = user.mind.get_skill_level(/datum/skill/blacksmithing)/2 + itemqualitymax //Makes sure that better smiths can make better items, even with a worse anvil. Encourages actually levelling up, instead of meta-rushing antag gear
	if(!artifactrolled)
		artifactchance = (1+(user.mind.get_skill_level(/datum/skill/blacksmithing)/4))/1500 //Reduced from 2500 temporarily. Should help that percentage get above 1% on the RAND()
	...
	if((currentsteps > 10 || (rng && prob(finalfailchance))) && !artifact)
		...
		user.mind.adjust_experience(/datum/skill/blacksmithing, 20, silent = FALSE)
	for(var/i in smithrecipes)
		if(i == stepsdone)
			...
			var/finalexperience = (15 *(stepexperience + finisheditem.quality))/5 //A total of 16x the amount of EXP at MAX, with a minimum gain of 15, Keep in mind that this is of course only possible with a max-tier anvil and an already insanely high level. Just makes earlier levels faster.
			if(currentquality <= 1)
				user.mind.adjust_experience(/datum/skill/blacksmithing, 40, silent = FALSE) //Incentivises not spamming Slag
			else
				user.mind.adjust_experience(/datum/skill/blacksmithing, finalexperience, silent = FALSE) //Made more forgiving for lower levels and terrible anvils.

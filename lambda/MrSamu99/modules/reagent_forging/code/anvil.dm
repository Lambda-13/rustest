/obj/structure/reagent_anvil
	...
	icon = 'lambda/MrSamu99/modules/reagent_forging/icons/obj/forge_structures.dmi'
...
/obj/structure/reagent_anvil/welder_act(mob/living/user, obj/item/I)
	. = ..()
...
/obj/structure/reagent_anvil/hammer_act(mob/living/user, obj/item/tool)
	...
	playsound(src, 'lambda/MrSamu99/modules/reagent_forging/sound/forge.ogg', 50, TRUE, ignore_walls = FALSE)
	...
	if(locate_obj && (locate_obj.obj_flags & ANVIL_REPAIR))
		if(locate_obj.obj_integrity >= locate_obj.max_integrity)
			balloon_alert(user, "already repaired")
			return TRUE

		while(locate_obj.obj_integrity < locate_obj.max_integrity)
			if(!do_after(user, 1 SECONDS, src))
				balloon_alert(user, "stopped repairing")
				return TRUE

			locate_obj.obj_integrity = locate_obj.obj_integrity + 10
...
/obj/structure/reagent_anvil/onZImpact(turf/impacted_turf, levels, message = TRUE)
	...
	if(istype(poor_target, /mob/living/carbon)) //If this mob is a carbon, break a few of their limbs
		poor_target.take_bodypart_damage(40 * levels)
		poor_target.take_bodypart_damage(40 * levels)

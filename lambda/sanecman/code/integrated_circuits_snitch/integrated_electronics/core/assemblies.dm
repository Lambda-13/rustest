...
/obj/item/electronic_assembly/Initialize(mapload)
	...
	set_custom_materials(list(SSmaterials.GetMaterialRef(/datum/material/iron) = round((max_complexity + max_components) / 4) * SScircuit.cost_multiplier))
...
//	inhand_icon_state = "circuitgun" //Спрайт в руке

...
/datum/overmap/ship/controlled
	...
	var/join_mode_ru = SHIP_JOIN_MODE_OPEN_RU
...
/datum/overmap/ship/controlled/start_dock(datum/overmap/to_dock, datum/docking_ticket/ticket)
	...
	refresh_engines()
	priority_announce("Начинаем процедуру стыковки, до окончания [dock_time/10] секунд.", "Автоматическая система судна", sender_override = name, zlevel = shuttle_port.virtual_z())

...
/obj/item/reagent_containers/glass/rag/afterattack(atom/target as obj|turf|area, mob/user,proximity)
	...
	if(iscarbon(target) && target.reagents && reagents.total_volume)
		...
		if(user.a_intent == INTENT_HARM && !C.is_mouth_covered())
			...
			C.visible_message("<span class='danger'>[user] smothers \the [C] with [src]!</span>", "<span class='userdanger'>[user] smothers you with [src]!</span>", "<span class='hear'>You hear some struggling and muffled cries of surprise.</span>")
			log_combat(user, C, "smothered", src, log_object)
		else
			reagents.expose(C, TOUCH)
			reagents.clear_reagents()
			C.visible_message("<span class='notice'>[user] touches \the [C] with [src].</span>")

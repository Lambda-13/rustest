...
/obj/item/gun/ballistic/revolver/russian/afterattack(atom/target, mob/living/user, flag, params)
	...
	if(ishuman(user))
		...
		if(!spun)
			to_chat(user, "<span class='warning'>You need to spin [src]'s chamber first!</span>")
			return
...
/obj/item/gun/ballistic/revolver/russian/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	playsound(src, dry_fire_sound, 30, TRUE)
	user.visible_message("<span class='danger'>[user.name] tries to fire [src] at the same time, but only succeeds at looking like an idiot.</span>", "<span class='danger'>[src]'s anti-combat mechanism prevents you from firing it at the same time!</span>")

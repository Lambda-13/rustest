...
/obj/item/modular_computer/ui_interact(mob/user, datum/tgui/ui)
	...
	if(!hard_drive || !hard_drive.stored_files || !hard_drive.stored_files.len)
		to_chat(user, "<span class='danger'>[src] beeps three times, it's screen displaying a \"DISK ERROR\" warning.</span>")
	...
	switch(action)
		...
		if("PC_runprogram")
			...
			if(!P || !istype(P)) // Program not found or it's not executable program.
				to_chat(user, "<span class='danger'>[src]'s screen shows \"I/O ERROR - Unable to run program\" warning.</span>")
				return
			...
			if(idle_threads.len > PU.max_idle_programs)
				to_chat(user, "<span class='danger'>[src] displays a \"Maximal CPU load reached. Unable to run another program.\" error.</span>")
				return

			if(P.requires_ntnet && !get_ntnet_status(P.requires_ntnet_feature)) // The program requires NTNet connection, but we are not connected to NTNet.
				to_chat(user, "<span class='danger'>[src]'s screen shows \"Unable to connect to NTNet. Please retry. If problem persists contact your system administrator.\" warning.</span>")
				return

...
/obj/machinery/microwave/examine(mob/user)
	. = ..()
	if(!operating)
		. += "<hr><span class='notice'>Alt-клик [src] to turn it on.</span>"

	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		. += "<hr><span class='warning'>You're too far away to examine [src]'s contents and display!</span>"
		return
	if(operating)
		. += "<hr><span class='notice'>[src] is operating.</span>"
		return

	if(length(ingredients))
		if(issilicon(user))
			. += "<hr><span class='notice'>[src] camera shows:</span>"
		else
		...
		for(var/O in items_counts)
			. += "<hr><span class='notice'>- [items_counts[O]]x [O].</span>"
	else
		. += "<hr><span class='notice'>[src] is empty.</span>"
...
/obj/machinery/microwave/attackby(obj/item/O, mob/user, params)
	...
	if(broken > 0)
		if(broken == 2 && O.tool_behaviour == TOOL_WIRECUTTER) // If it's broken and they're using a screwdriver
			...
			if(O.use_tool(src, user, 20))
				user.visible_message("<span class='notice'>[user] fixes part of [src].</span>", "<span class='notice'>You fix part of [src].</span>")
				broken = 1 // Fix it a bit
		else if(broken == 1 && O.tool_behaviour == TOOL_WELDER) // If it's broken and they're doing the wrench
			user.visible_message("<span class='notice'>[user] starts to fix part of [src].</span>", "<span class='notice'>You start to fix part of [src]...</span>")
			if(O.use_tool(src, user, 20))
				user.visible_message("<span class='notice'>[user] fixes [src].</span>", "<span class='notice'>You fix [src].</span>")
				broken = 0
				update_icon()
	...
	if(istype(O, /obj/item/reagent_containers/spray))
		...
		if(clean_spray.reagents.has_reagent(/datum/reagent/space_cleaner, clean_spray.amount_per_transfer_from_this))
			...
			user.visible_message("<span class='notice'>[user] cleans [src].</span>", "<span class='notice'>You clean [src].</span>")
			dirty = 0
			update_icon()
		else
	...
	if(dirty == 100) // The microwave is all dirty so can't be used!
		to_chat(user, "<span class='warning'>[src] is dirty!</span>")
		return TRUE
	...
	if(istype(O, /obj/item/storage/bag/tray))
		...
		for(var/obj/item/reagent_containers/food/snacks/S in T.contents)
			if(ingredients.len >= max_n_of_items)
				to_chat(user, "<span class='warning'>[src] полон you can't put anything in!</span>")
				return TRUE
	...
	if(O.w_class <= WEIGHT_CLASS_NORMAL && !istype(O, /obj/item/storage) && user.a_intent == INTENT_HELP)
		...
		if(!user.transferItemToLoc(O, src))
			to_chat(user, "<span class='warning'>\The [O] прилип к рукеr hand!</span>")
			return FALSE

		ingredients += O
		user.visible_message("<span class='notice'>[user] adds \a [O] to [src].</span>", "<span class='notice'>You add [O] to [src].</span>")
		return
...
/obj/machinery/microwave/proc/wzhzhzh()
	visible_message("<span class='notice'>[src] turns on.</span>", null, "<span class='hear'>You hear a microwave humming.</span>")
	operating = TRUE

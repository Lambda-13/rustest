...
/client/New(TopicData)
	...
	if (mob && reconnecting)
		...
		if (!stealth_admin)
			deadchat_broadcast(" переподключился.", "<b>[mob][mob.get_realname_string()]</b>", follow_target = mob, turf_target = get_turf(mob), message_type = DEADCHAT_LOGIN_LOGOUT, admin_only=!announce_leave)

...
/atom/movable/screen/movable/action_button/MouseDrop(over_object)
	...
	if((istype(over_object, /atom/movable/screen/movable/action_button) && !istype(over_object, /atom/movable/screen/movable/action_button/hide_toggle)))
		if(locked)
			to_chat(usr, "<span class='warning'>Кнопка действия \"[name]\" заблокирована, разблокируйте для начала.</span>")
			return
...
/atom/movable/screen/movable/action_button/Click(location,control,params)
	...
	if(LAZYACCESS(modifiers, CTRL_CLICK))
		locked = !locked
		to_chat(usr, "<span class='notice'>Кнопка действия \"[name]\" [locked ? "зв" : "раз"]блокирована.</span>")
...
/atom/movable/screen/movable/action_button/hide_toggle
	name = "скрыть кнопки"
	desc = "Щелкните по любой кнопке, удерживая клавишу Shift, чтобы сбросить ее положение, и щелкните ее, удерживая клавишу Ctrl, чтобы зафиксировать на месте. Удерживая нажатой клавишу Alt, нажмите на любую кнопку, чтобы сбросить все кнопки в исходное положение."
...
/atom/movable/screen/movable/action_button/hide_toggle/Click(location,control,params)
	...
	if(LAZYACCESS(modifiers, CTRL_CLICK))
		locked = !locked
		to_chat(usr, "<span class='notice'>Кнопка действия \"[name]\" [locked ? "за" : "раз"]блокирована.</span>")
		if(id && usr.client) //try to (un)remember position
			usr.client.prefs.action_buttons_screen_locs["[name]_[id]"] = locked ? moved : null
		return TRUE
	if(LAZYACCESS(modifiers, ALT_CLICK))
		...
		usr.update_action_buttons(TRUE)
		to_chat(usr, "<span class='notice'>Положения кнопки действия были сброшены.</span>")
		return TRUE
...
/atom/movable/screen/movable/action_button/hide_toggle/AltClick(mob/user)
	...
	to_chat(user, "<span class='notice'>Положения кнопки действия были сброшены.</span>")

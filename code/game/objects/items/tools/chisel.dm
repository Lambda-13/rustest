...
/obj/item/chisel/pre_attack(atom/target, mob/living/user, params)
	...
	if(toggling_smooth)
		if(!atom_supports_smoothing(target))
			to_chat(user, "<span class='warning'>[src] makes a tart buzz. \the [target] doesn't appear to support smoothing.</span>")
			return ..()
		smooth_atom(target, user)
	else
		if(!atom_supports_diagonal(target))
			to_chat(user, "<span class='warning'>[src] makes a tart buzz. \the [target] doesn't appear to support smoothed corners.</span>")
			return ..()
...
/obj/item/chisel/attack_self(mob/user)
	...
	to_chat(user, "<span class='notice'>[src] is now set to reform the [(toggling_smooth ? "smoothness" : "corners")] of objects.</span>")

/obj/item/chisel/proc/smooth_atom(atom/target, mob/living/user)
	target.smoothing_flags ^= SMOOTH_BITMASK
	to_chat(user, "<span class='notice'>[src] vibrates gently as it reforms \the [target] to be [((target.smoothing_flags & SMOOTH_BITMASK) ? "smooth" : "rough")].</span>")
...
/obj/item/chisel/proc/smooth_atom_diagonal(atom/target, mob/living/user)
	...
	to_chat(user, "<span class='notice'>[src] vibrates intensely as it reforms \the [target]'s corners.</span>")

/obj/machinery/mecha_part_fabricator
	...
	desc = "В данный момент простаивает."
...
/obj/machinery/mecha_part_fabricator/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<hr><span class='notice'>Дисплей показывает: Вмещает всего <b>[rmat.local_size]</b> материалов.<br>Расход материала <b>[component_coeff*100]%</b>.<br>Время создания сокращено на <b>[100-time_coeff*100]%</b>.</span>"
...
/obj/machinery/mecha_part_fabricator/emag_act()
	...
	obj_flags |= EMAGGED
	req_access = list()
	say("Ошибка БД \[Fx[rand(1000,1000000)]\]")
	sleep(10)
	say("Запуск автовосстановления...")
	sleep(15)
	say("Ошибка БД \[Fx[rand(1000,1000000)]\]. Начинаю загрузку пользовательской БД...")
	sleep(30)
	say("Пользовательская БД загружена. Пожалуйста, свяжитесь с вашим оператором системы для получения помощи.")
	...
	for(var/v in stored_research.researched_designs)
		...
		if(D.build_type & MECHFAB)
			...
			if(check_resources(D))
				output += "<a href='?src=[REF(src)];part=[D.id]'>Создать</a> | "
			output += "<a href='?src=[REF(src)];add_to_queue=[D.id]'>Добавить в очередь</a><a href='?src=[REF(src)];part_desc=[D.id]'>?</a></div>"
	return output

/obj/machinery/mecha_part_fabricator/proc/output_part_info(datum/design/D)
	var/output = "<meta charset='utf-8'>[initial(D.name)] (Cost: [output_part_cost(D)]) [get_construction_time_w_coeff(D)/10] с."
	return output
...
/obj/machinery/mecha_part_fabricator/proc/output_ui_header()
	var/output
	output += "<meta charset='utf-8'>"
	output += "<div class='statusDisplay'><b>Фабрикатор</b><br>"
	output += "Защитные протоколы: [(obj_flags & EMAGGED)? "<font color='red'>ОТКЛЮЧЕНЫ</font>" : "<font color='green'>ВКЛЮЧЕНЫ</font>"]<br>"
	if (rmat.mat_container)
		output += "<a href='?src=[REF(src)];screen=resources'><B>Материал:</B> [rmat.format_amount()]</A>"
	else
		output += "<font color='red'>Рудный бункер не подключён, свяжитесь с вашим завхозом.</font>"
	output += "<br><a href='?src=[REF(src)];sync=1'>Синхронизация с сервером</a><br>"
	output += "<a href='?src=[REF(src)];screen=main'>Главное меню</a>"
	output += "</div>"
...
/obj/machinery/mecha_part_fabricator/proc/output_ui_materials()
	...
	for(var/mat_id in rmat.mat_container.materials)
		...
		if(amount >= MINERAL_MATERIAL_AMOUNT) output += "<A href='?src=[REF(src)];remove_mat=1;material=[ref]'>Извлечь</A>"
		if(amount >= MINERAL_MATERIAL_AMOUNT*5) output += "<A href='?src=[REF(src)];remove_mat=5;material=[ref]'>5х</A>"
		if(amount >= MINERAL_MATERIAL_AMOUNT) output += "<A href='?src=[REF(src)];remove_mat=50;material=[ref]'>Всё</A>"
		output += "<br>"
	output += "</div>"
	return output
...
/obj/machinery/mecha_part_fabricator/proc/output_ui_search()
	var/output
	output += "<meta charset='utf-8'>"
	output += "<h2>Результаты поиска:</h2>"
...
/obj/machinery/mecha_part_fabricator/proc/build_part(datum/design/D)
	...
	var/datum/component/material_container/materials = rmat.mat_container
	if (!materials)
		say("Нет доступа к рудному бункеру.")
		return FALSE
	if (rmat.on_hold())
		say("Доступ к ресурсам остановлен.")
		return FALSE
	if(!check_resources(D))
		say("Очередь задач остановлена - не хватает ресурсов.")
		return FALSE
	being_built = D
	desc = "Он создаёт [initial(D.name)]."
...
/obj/machinery/mecha_part_fabricator/proc/process_queue()
	...
	say("Список задач выполнен.")

/obj/machinery/mecha_part_fabricator/proc/list_queue()
	var/output = "<meta charset='utf-8'><b>Список задач:</b>"
	if(!istype(queue) || !queue.len)
		output += "<br>Пуст - добавьте что-то в очередь."
	else
		output += "<ol>"
		var/i = 0
		for(var/datum/design/D in queue)
			...
			output += "<a href='?src=[REF(src)];remove_from_queue=[i]'>Извлечь</a></li>"

		output += "</ol>"
		output += "<a href='?src=[REF(src)];process_queue=1'>Список задач</a> | <a href='?src=[REF(src)];clear_queue=1'>Очистить список задач</a>"
	return output
...
/obj/machinery/mecha_part_fabricator/proc/sync()
	for(var/obj/machinery/computer/rdconsole/RDC in oview(7,src))
		...
		updateUsrDialog()
		say("Синхронизация с сервером произведена.")
		return
	temp = "<meta charset='utf-8'>Нет доступа к серверу РнД.<br>Подключите сервер и повторите попытку.<br><a href='?src=[REF(src)];clear_temp=1'>🔁</a>"
	updateUsrDialog()
	return
...
/obj/machinery/mecha_part_fabricator/ui_interact(mob/user)
	...
	if(exit.density)
		say("Выход произведёных вещей заблокирован, отмена.")
		return
	if(temp)
		left_part = temp
	else if(being_built)
		var/obj/I = being_built.build_path
		left_part = {"<div class='statusDisplay'>Производим [initial(I.name)].<BR>
							Ожидайте окончания...</div>"}
	else
		left_part = output_ui_header()
		switch(screen)
			if("main")
				for(var/part_set in part_sets)
					left_part += "<a href='?src=[REF(src)];part_set=[part_set]'>[part_set]</a> - <a href='?src=[REF(src)];partset_to_queue=[part_set]'>Добавить все части в список задач</a><br>"
			if("parts")
	...
	if(href_list["part_desc"])
		...
		for(var/v in stored_research.researched_designs)
			...
			if(D.build_type & MECHFAB)
				if(D.id == T)
					var/obj/part = D.build_path
					temp = {"<h1>Описание [initial(part.name)]:</h1>
								[initial(part.desc)]<br>
								<a href='?src=[REF(src)];clear_temp=1'>🔁</a>
								"}
					break
...
/obj/machinery/mecha_part_fabricator/proc/eject_sheets(eject_sheet, eject_amt)
	...
	if (!mat_container)
		...
		return 0
	if (rmat.on_hold())
		say("Доступ к рудному бункеру приостановлен.")
		return 0
...
/obj/machinery/mecha_part_fabricator/proc/is_insertion_ready(mob/user)
	if(panel_open)
		to_chat(user, "<span class='warning'>Не могу вставить [src] пока техническая панель открыта!</span>")
		return FALSE
	if(being_built)
		to_chat(user, "<span class='warning'>[src] сейчас занят, похоже стоит дождаться завершения работ.</span>")
		return FALSE

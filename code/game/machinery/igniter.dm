...
/obj/machinery/sparker/attackby(obj/item/W, mob/user, params)
	if (W.tool_behaviour == TOOL_SCREWDRIVER)
		...
		if (src.disable)
			user.visible_message("<span class='notice'>[user] disables [src]!</span>", "<span class='notice'>You disable the connection to [src].</span>")
		if (!src.disable)
			user.visible_message("<span class='notice'>[user] reconnects [src]!</span>", "<span class='notice'>You fix the connection to [src].</span>")
		update_icon()
	else
		return ..()

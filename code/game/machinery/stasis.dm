...
/obj/machinery/stasis/examine(mob/user)
	. = ..()
	. += "<hr><span class='notice'>Alt-клик to [stasis_enabled ? "turn off" : "turn on"] the machine.</span>"
	. += "<hr><span class='notice'>[src] is [op_computer ? "linked" : "<b>NOT</b> linked"] to a nearby operating computer.</span>"
...
/obj/machinery/stasis/AltClick(mob/user)
	if(world.time >= stasis_can_toggle && user.canUseTopic(src, !issilicon(user)))
		...
		user.visible_message("<span class='notice'>[src] [stasis_enabled ? "powers on" : "shuts down"].</span>", \
					"<span class='notice'>You [stasis_enabled ? "power on" : "shut down"] [src].</span>", \
